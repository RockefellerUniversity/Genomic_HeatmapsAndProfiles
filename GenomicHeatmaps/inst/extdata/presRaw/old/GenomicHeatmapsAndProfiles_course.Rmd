...
We can retrieve raw and processed data for GSM4332935, GSM4332934 and GSM4332944 directly from [GEO](https://www.ncbi.nlm.nih.gov/geo/query/acc.cgi?acc=GSE145783).
...
myRes <- import_list("~/Downloads/1-s2.0-S1550413120301236-mmc5.xlsx",
                     which=c("Consensus ZBTB1 Peaks"))
ZBTB1peaks <- myRes[[1]][-1,1:3]
colnames(ZBTB1peaks) <- colnames(ZBTB1peaks) <- c("seqnames","start","end")
require(GenomicRanges)
ZBTB1GR <- GRanges(ZBTB1peaks)
names(ZBTB1GR) <- as.character(ZBTB1GR)
dir.create("Beds",showWarnings = FALSE,recursive = TRUE)
blacklist <- rtracklayer::import("~/Downloads/ENCFF001TDO.bed.gz")

rtracklayer::export(ZBTB1GR,con="Beds/ZBTB1Peaks.bed")
```
...
```{r makeBed2,echo=FALSE,eval=TRUE,cache=TRUE}
myRes <- import_list("~/Downloads/1-s2.0-S1550413120301236-mmc4.xlsx")
require(GenomicRanges)
ATF4ANDZBTB1peaks <- myRes[[1]][-1,1:3]
# ZBTB1peaks <- myRes[[2]][-1,1:3]
colnames(ATF4ANDZBTB1peaks) <- c("seqnames","start","end")

ATF4ANDZBTB1peaks <- GRanges(ATF4ANDZBTB1peaks)
names(ATF4ANDZBTB1peaks) <- as.character(ATF4ANDZBTB1peaks)
rtracklayer::export(ATF4ANDZBTB1peaks,con="Beds/ATF4ANDZBTB1_Within2000kb.bed")
```
...
require(magrittr)
ztbtb1Andatf4_Grouped <- groupBy(ztbtb1Andatf4_hclust,
                                 ATF4ANDZBTB1peaks_GR,
                                 include_nonoverlapping = TRUE)
generateEnrichedHeatmap(ztbtb1Andatf4_Grouped,
...
```{r,echo=TRUE,eval=FALSE}
require(magrittr)
longFrame <- summarize(ztbtb1Andatf4_Grouped,
                        rowSums,
                        output="long",sampleData_columns_for_longPlot = "Antibody")
...
require(ggplot2)
ggplot(longFrame,
       aes(y=Signal,x=Sample,fill=GR_overlap_names))+facet_wrap(Antibody~.,scales = "free")+
...
ztbtb1Andatf4_Grouped_anno <- annotateRanges(ztbtb1Andatf4_Grouped,
...
generateEnrichedHeatmap(ztbtb1Andatf4_Grouped,
                        all_color_scales_equal = FALSE,
                        genes_to_label = c("ASNS"))

```

```{r,echo=TRUE,eval=FALSE}
ztbtb1Andatf4_Grouped_Great <- annotateRanges_great(ztbtb1Andatf4_Grouped,species = "hg19")
generateEnrichedHeatmap(ztbtb1Andatf4_Grouped_Great,
                        all_color_scales_equal = FALSE,
                        genes_to_label = c("ASNS"))

ztbtb1Andatf4_Grouped_Great <- summarize(ztbtb1Andatf4_Grouped,
                                          rowSums,
                                          output="object")


export_deepToolsMat(ztbtb1Andatf4_Grouped_Great,con = "~/Downloads/Test_Deeptools.MAT")
# plotHeatmap -m ~/Downloads/Test_Deeptools.MAT.gz -o ~/Downloads/Test_DP.png --zMax 3 3 1 --zMin 0.4 0.4 0.3 --colorList white,darkred white,darkred white,black 
```
# Contact
